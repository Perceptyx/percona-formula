...
{%- set percona_toolkit_pkg_url = salt['cmd.run_stdout']('curl -sL ' ~ percona_toolkit_url ~ ' | grep -oP "https://downloads[^\s>]+percona-toolkit_([0-9])+[^\s]+.' ~ grains['oscodename'] ~ '_' ~ mysql.os_arch ~ '\.deb" | tail -1 ', python_shell=True) %}
